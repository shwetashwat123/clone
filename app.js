	...
	for(let i = 0; i<currentMovieStack.length; i++){
		...
		<div class="movie-title-container">
			...
			<div class="rating-container">
				<img src="https://static-00.iconduck.com/assets.00/rating-icon-512x488-f3wudmx0.png" alt="">
		...
		<button onclick="${actionForButton}(this)" class="add-to-favourite-button text-icon-button" data-id="${currentMovieStack[i].id}" >
			<img src="https://w7.pngwing.com/pngs/79/141/png-transparent-yellow-star-illustration-dyersburg-computer-icons-web-page-directory-favorites-star-icon-miscellaneous-television-orange-thumbnail.png">
	...
	movieDetailCard.innerHTML = `
		...
		<div class="detail-movie-title">
			...
			<div class="detail-movie-rating">
				<img src="https://static-00.iconduck.com/assets.00/rating-icon-512x488-f3wudmx0.png">
...
function remove(element){
	...
	let favouriteMoviesShwetashwat = JSON.parse(localStorage.getItem("favouriteMoviesShwetashwat"));
	let newFavouriteMovies = [];
	for(let i = 0; i<favouriteMoviesShwetashwat.length; i++){
		if(favouriteMoviesShwetashwat[i].id == id){
			continue;
		}
		newFavouriteMovies.push(favouriteMoviesShwetashwat[i]);
	}
	
	localStorage.setItem("favouriteMoviesShwetashwat", JSON.stringify(newFavouriteMovies));
...
goToFavouriteButton.addEventListener('click', ()=>{
	...
	let favouriteMoviesShwetashwat = JSON.parse(localStorage.getItem("favouriteMoviesShwetashwat"));
	if(favouriteMoviesShwetashwat == null || favouriteMoviesShwetashwat.length < 1){
		showAlert("you have not added any movie to favourite");
		return;
	}

	 currentMovieStack = favouriteMoviesShwetashwat;
	renderList("remove");
})
...
function favourite(element){
	...
	for(let i = 0; i< currentMovieStack.length; i++){
		if(currentMovieStack[i].id == id){
			...
			if(favouriteMoviesShwetashwat == null){
				favouriteMoviesShwetashwat = [];
			}

			favouriteMoviesShwetashwat.unshift(currentMovieStack[i]);
			localStorage.setItem("favouriteMoviesShwetashwat", JSON.stringify(favouriteMoviesShwetashwat));
